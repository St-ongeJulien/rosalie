...
public static class HytaleGuideItemsService
{
    ...
    private static readonly Uri BundledItemsJsonUri = new("avares://Hylterium.QuestStudio/Assets/items.generated.json");
    ...
    public static async Task<List<GameItemEntry>> LoadBundledAsync()
    {
        try
        {
            // Prefer JSON if present (faster + more stable than parsing HTML)
            if (AssetLoader.Exists(BundledItemsJsonUri))
            {
                await using var jsonStream = AssetLoader.Open(BundledItemsJsonUri);
                using var jsonReader = new System.IO.StreamReader(jsonStream);
                var json = await jsonReader.ReadToEndAsync();

                // Accept both our exported shape ({ itemId, name, imageUrl }) and a raw GameItemEntry list.
                var itemsFromJson = ItemsCatalogService.LoadFromJsonString(json);
