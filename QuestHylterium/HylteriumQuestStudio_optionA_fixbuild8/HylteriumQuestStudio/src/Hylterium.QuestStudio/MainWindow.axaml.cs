...
private static IEnumerable<Inline> ParseRichTextTags(string text)
{
    ...
    void flushSegment(List<Inline> list, string seg)
    {
        ...
        for (var i = 0; i < parts.Length; i++)
        {
            if (parts[i].Length > 0)
            {
	        			// Use Span wrapper: more reliable across themes than styling Run directly
                    if (state.Bold || state.Italic || state.Foreground is not null)
                    {
                        var span = new Span();
                        if (state.Bold) span.FontWeight = FontWeight.Bold;
                        if (state.Italic) span.FontStyle = FontStyle.Italic;
                        if (state.Foreground is not null) span.Foreground = state.Foreground;
                        span.Inlines.Add(new Run(parts[i]));
                        list.Add(span);
                    }
                    else
                    {
                        list.Add(new Run(parts[i]));
                    }
            }
