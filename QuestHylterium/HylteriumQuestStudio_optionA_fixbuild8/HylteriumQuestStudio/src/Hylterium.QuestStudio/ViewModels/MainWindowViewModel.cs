...
// --- Assets ---
    ...
    private void ApplyItemFilter()
    {
        ...
        IEnumerable<GameItemEntry> list;
        ...
        {
            // "...plus proche" : on garde seulement les matchs et on les ordonne par score.
            list = Items
                .Select(i => new { Item = i, Score = ComputeItemMatchScore(i, term) })
                .Where(x => x.Score < 100000)
                .OrderBy(x => x.Score)
    ...
    private static int ComputeItemMatchScore(GameItemEntry item, string term)
    {
        ...
        if (id == q) best = 0;
        if (name == q) best = Math.Min(best, 1);

        if (id.StartsWith(q)) best = Math.Min(best, 5 + (id.Length - q.Length));
        if (name.StartsWith(q)) best = Math.Min(best, 10 + (name.Length - q.Length));

        var idx = id.IndexOf(q, StringComparison.Ordinal);
        if (idx >= 0) best = Math.Min(best, 50 + idx);

        idx = name.IndexOf(q, StringComparison.Ordinal);
        if (idx >= 0) best = Math.Min(best, 80 + idx);

        return best == int.MaxValue ? 100000 : best;
    }
...
// --- Helpers for InfoBar ---
    ...
    private void DetectWizardDefaults()
    {
        ...
        if (npc is null)
        {
            WizardStatusHint = "Choisis un NPC à gauche, puis utilise l'assistant pour créer/relier tes quêtes automatiquement.";
            return;
        }
        ...
        var questId = $"{QuestWizardPrefix}{QuestWizardNumber}";
        var prevQuestId = QuestWizardNumber > 1 ? $"{QuestWizardPrefix}{QuestWizardNumber - 1}" : "(aucune)";
        var nextQuestId = $"{QuestWizardPrefix}{QuestWizardNumber + 1}";

        WizardStatusHint = $"Auto : prochaine = {questId} | prérequis suggéré = {prevQuestId} | quand tu créeras {nextQuestId}, {questId} sera rendu disponible automatiquement.";
    }
    ...
    // Auto-chain lock flag stored in page.Extra["autoChainLocked"] (JsonExtensionData)
    private static bool IsAutoChainLocked(DialogPage? page)
    {
        if (page?.Extra is null) return false;
        if (page.Extra.TryGetValue("autoChainLocked", out var el))
        {
            try
            {
                return el.ValueKind switch
                {
                    ...
                    System.Text.Json.JsonValueKind.String => string.Equals(el.GetString(), "true", StringComparison.OrdinalIgnoreCase),
                    _ => false
                };
            }
            catch { }
    ...
    private static void SetAutoChainLocked(DialogPage? page, bool locked)
    {
        if (page is null) return;
        page.Extra ??= new Dictionary<string, System.Text.Json.JsonElement>();

        var json = locked ? "true" : "false";
        page.Extra["autoChainLocked"] = System.Text.Json.JsonDocument.Parse(json).RootElement.Clone();
    }

    public bool SelectedPageAutoChainLocked
    {
        get => IsAutoChainLocked(SelectedPage);
        set
        {
            if (SelectedPage is null) return;
            SetAutoChainLocked(SelectedPage, value);
            OnPropertyChanged(nameof(SelectedPageAutoChainLocked));
        }
    }
...
private DialogPage? FindPageByQuestId(NpcDef npc, string questId)
    ...
    [RelayCommand]
    private void ApplyAutoOrderToSelectedNpc()
    {
        if (SelectedNpc is null)
        {
            SetInfo("Sélectionne un NPC.", "Assistant", InfoBarSeverity.Warning);
            return;
        }

        // Take quest pages in file order, excluding those locked by the user.
        var chainPages = SelectedNpc.Pages
            .Select(p => (page: p, questId: DetectQuestIdFromPage(p)))
            .Where(t => !string.IsNullOrWhiteSpace(t.questId))
            .Where(t => !IsAutoChainLocked(t.page))
            .ToList();

        if (chainPages.Count == 0)
        {
            SetInfo("Aucune page de quête détectée (quest:*), ou toutes sont verrouillées.", "Assistant", InfoBarSeverity.Warning);
            return;
        }

        for (int i = 0; i < chainPages.Count; i++)
        {
            var (page, questId) = chainPages[i];
            var nextQuestId = i < chainPages.Count - 1 ? chainPages[i + 1].questId : null;
            var isFirst = i == 0;

            ApplyFernandPatternToPage(page, questId!, nextQuestId, isFirst);
        }
        ...
        SetInfo("Chaîne appliquée automatiquement selon l'ordre du fichier (les pages verrouillées sont ignorées).", "Assistant");
    }
...
[RelayCommand]
    ...
    private void ApplyFernandPatternToSelectedPage()
    {
        ...
        {
            SetInfo("Sélectionne un NPC et une page.", "Assistant");
            return;
        }
        ...
        {
            SetInfo("Aucun questId détecté sur cette page (quest:*).", "Assistant", InfoBarSeverity.Warning);
            return;
        }
        ...
        SetInfo("Pattern appliqué sur la page sélectionnée.", "Assistant");
    }
    ...
    private void WizardCreateNextQuest()
    {
        ...
        {
            SetInfo("Prefix invalide.", "Assistant", InfoBarSeverity.Warning);
            return;
        }
        ...
        {
            SetInfo($"La quête existe déjà: {questId}", "Assistant", InfoBarSeverity.Warning);
            return;
        }
...
{(rep > 0 ? $"{{#6aa84f}}{{b}}Récompense : +{rep} réputation{{/}}{{/}}" : "")} »";
        ...
        SetInfo($"Quête créée: {questId} (pageId: {page.PageId})", "Assistant");
        DetectWizardDefaults();
    }
    ...
    private void WizardRepairChain()
    {
        ...
        if (map.Count == 0)
        {
            SetInfo("Aucune quête détectée pour ce prefix sur ce NPC.", "Assistant", InfoBarSeverity.Warning);
            return;
        }
        ...
        SetInfo($"Chaîne réparée (pattern) pour prefix: {prefix}", "Assistant");
        DetectWizardDefaults();
    }}
