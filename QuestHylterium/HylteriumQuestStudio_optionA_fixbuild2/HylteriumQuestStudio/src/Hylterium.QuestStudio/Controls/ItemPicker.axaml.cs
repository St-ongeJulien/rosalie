...
public partial class ItemPicker : UserControl
{
    public ItemPicker()
    {
        ...
        this.GetObservable(QueryProperty).Subscribe(_ => OpenAndRefresh());
        this.GetObservable(ItemsSourceProperty).Subscribe(_ => OpenAndRefresh());
        this.GetObservable(SelectedItemIdProperty).Subscribe(_ => SyncFromSelectedId());
    }
